...
// #[cfg(windows)]
// pub fn run_command_silent(shell: &str, arguments: Vec<&str>, command: &str) -> std::result::Result<(), clap::Error> {
...
pub fn run_command(shell: &str, arguments: Vec<&str>, command: &str) -> std::result::Result<(), clap::Error> {
    ...
    println!("{} {}", shell, arguments.iter().fold(String::new(), |acc, &num| acc + &num.to_string() + " "));
    let mut child_process = std::process::Command::new(shell)
        .stdin(Stdio::piped())
        .args(arguments)
        .spawn().unwrap();
...
pub fn start_terminal(command: &str) {
    ...
    let mut arguments: Vec<&str> = [].to_vec();
    arguments.push("powershell");
    arguments.push("-NoExit");
    arguments.push("-Command");

    run_command("wt.exe", arguments, script.as_str());
}
